...
function SettingsView({ data, refresh, toast }: { data: DashboardPayload; refresh: () => Promise<void>; toast: (message: string, error?: boolean) => void }) {
  return <><section className="page-heading"><div><span className="eyebrow">INTEGRATIONS</span><h1>接口与机器人设置</h1><p>配置分析模型和飞书机器人，保存后系统会自动完成消息接收、分析、归档与报告返回。</p></div></section><div className="security-banner"><AlertCircle /><div><strong>所有密钥都只在设置页面填写</strong><p>密钥会加密保存在本机，不写入源码。已经发到聊天里的旧密钥建议撤销并重新生成。</p></div></div><FeishuSettingsCard toast={toast} /><div className="provider-grid">{data.providers.map((setting) => <ProviderCard key={setting.provider} setting={setting} refresh={refresh} toast={toast} />)}</div><section className="routing-card"><div><span className="eyebrow">QWEN ONLY</span><h2>统一分析路线</h2><p>TokScript 获取视频和公开数据；商品图文识别、语音转写、关键帧分析和中文报告全部由 Qwen 完成。</p></div><div className="routing-flow"><span>TokScript</span><ArrowRight /><span>Qwen 分析</span><ArrowRight /><strong>中文报告</strong></div></section></>;
}
