  ...
  return (
    <div className="app-shell">
      ...
      <div className="main-shell">
        <header className="topbar"><div className="global-search"><Search /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索产品、账号、文案…" /></div><div className="top-actions"><div className="provider-state">{data?.providers.map((provider) => <span key={provider.provider} className={provider.hasKey && provider.enabled ? "online" : ""}>{provider.provider === "qwen" ? "Qwen" : "TokScript"}</span>)}</div><button className="primary-action" onClick={() => setView("analyze")}><Plus />分析视频</button></div></header>
...
function ProviderCard({ setting, refresh, toast }: { setting: ProviderSetting; refresh: () => Promise<void>; toast: (message: string, error?: boolean) => void }) {
  ...
  const names = { tokscript: "TokScript MCP", qwen: "Qwen AI", openai: "OpenAI（可选）" };
  const details = { tokscript: "下载原始 MP4、封面和公开数据", qwen: "完整 MP4 画面与音轨、镜头结构和中文报告", openai: "商品图文或中文翻译的可选服务商" };
...
function SettingsView({ data, refresh, toast }: { data: DashboardPayload; refresh: () => Promise<void>; toast: (message: string, error?: boolean) => void }) {
  return <><section className="page-heading"><div><span className="eyebrow">INTEGRATIONS</span><h1>接口与机器人设置</h1><p>配置分析模型和飞书机器人，保存后系统会自动完成消息接收、分析、归档与报告返回。</p></div></section><div className="security-banner"><AlertCircle /><div><strong>密钥只保存在受保护的配置中</strong><p>本机平台密钥会加密保存；云端商品分析的 OpenAI 密钥由部署 Secret 注入，均不写入源码或日志。已经发到聊天里的旧密钥建议撤销并重新生成。</p></div></div><FeishuSettingsCard toast={toast} /><div className="provider-grid">{data.providers.map((setting) => <ProviderCard key={setting.provider} setting={setting} refresh={refresh} toast={toast} />)}</div><section className="routing-card"><div><span className="eyebrow">MULTI-MODEL</span><h2>分工分析路线</h2><p>TikTok Shop 商品详情页与主体图片由 OpenAI 分析；Qwen Omni 直接理解完整 MP4 的画面、口播、音乐和音效。</p></div><div className="routing-flow"><span>TikTok Shop 完整详情</span><ArrowRight /><span>OpenAI 商品分析</span><ArrowRight /><strong>产品手卡</strong></div></section></>;
}
