...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body.productId === "string" && (await getProduct(body.productId))) values.product_id = body.productId;
  if (typeof body.title === "string") values.title = body.title;
  const video = await updateVideo(id, values);
  if (video?.status === "completed") await learnFromVideo(id);
  return video ? NextResponse.json({ video }) : NextResponse.json({ error: "视频不存在" }, { status: 404 });
}
