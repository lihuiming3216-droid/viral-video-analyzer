...
class TokScriptClient {
  ...
  private async post(method: string, params?: Record<string, unknown>, notification = false) {
    ...
    const response = await fetch(this.endpoint, { method: "POST", headers, body: JSON.stringify(body), signal: this.signal });
...
function findMediaUrl(root: unknown, kind: "video" | "cover") {
  const preferred = kind === "video"
    ? ["download_url", "downloadUrl", "video_url", "videoUrl", "play_url", "playUrl", "url"]
    : ["cover_url", "coverUrl", "thumbnail_url", "thumbnailUrl", "cover", "thumbnail", "url"];
